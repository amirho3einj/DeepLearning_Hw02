...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code
        dx = dout.clone() #we can use copy.deepcopy() too, but clone is using memory less than copy.deepcopy()
        dx[cache < 0] = 0
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
...
def softmax_loss(x, y):
    ...
    n = len(y)
    exp_x = torch.exp(x)
    x_softmax = exp_x / torch.sum(exp_x, dim=1, keepdim=True)
    loss = -torch.log(x_softmax[range(n), y]).mean()
    dx = x_softmax.clone()
    dx[range(n), y] -= 1
    dx /= n
    ######################################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #########################Forward#############################
        out_layer1, cache_layer1 = Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
        out_layer2, cache_layer2 = Linear.forward(out_layer1,self.params['W2'],self.params['b2'])
        scores = out_layer2
        # Forward pass
        # h1 = torch.relu(torch.matmul(X, self.params['W1']) + self.params['b1'])
        ...
        n = len(y)
        loss, dscores = softmax_loss(out_layer2,y)
        # Calcualate L2 loss
        reg_loss = self.reg * (torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(self.params['W2'] * self.params['W2']))
        #Add L2 loss
        loss += reg_loss
        # Backward pass

        dx2, dW2, db2 = Linear.backward(dscores, cache_layer2)
        dx1, dW1, db1 = Linear_ReLU.backward(dx2, cache_layer1)        
        # return dx, dw, db
        # dh1 = torch.matmul(dscores, torch.transpose(self.params['W2'], 0, 1))
        # dW2 = torch.matmul(torch.transpose(h1, 0, 1), dscores) + self.reg * self.params['W2']
        # db2 = torch.sum(dscores, dim=0)
        # dhidden = dh1 * (h1 > 0).float()
        # dW1 = torch.matmul(torch.transpose(X, 0, 1), dhidden) + self.reg * self.params['W1']
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
              self.params['W' + str(i+1)] = torch.randn(input_dim, hidden_dims[0], dtype=dtype, device=device) * weight_scale
              self.params['b' + str(i+1)] = torch.zeros(hidden_dims[0], dtype=dtype, device=device) 
            elif i == self.num_layers - 1:
              self.params['W' + str(i+1)] = torch.randn(hidden_dims[i-1], num_classes, dtype=dtype, device=device) * weight_scale
              self.params['b' + str(i+1)] = torch.zeros(num_classes, dtype=dtype, device=device)
            else:
              self.params['W' + str(i+1)] = torch.randn(hidden_dims[i-1], hidden_dims[i], dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers:  
              scores, cache[i] = Linear.forward(out[self.num_layers-1],self.params['W'+str(i)],self.params['b'+str(i)])
            else:
              out[i], cache[i] = Linear_ReLU.forward(out[i-1],self.params['W'+str(i)],self.params['b'+str(i)])
              if self.use_dropout:
                out[i] , cache['dropout'+str(i)]  = Dropout.forward(out[i],self.dropout_param)
                # mask = (torch.rand_like(out[i]) >= self.dropout_param['p']) / (1- self.dropout_param['p']) 
                # out[i] *= mask           
                # cache['dropout'+str(i)] = mask
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        # Calcualate L2 loss
        reg_loss = self.reg * sum(torch.sum(self.params['W' + str(i)] ** 2) for i in range(1, self.num_layers + 1))
        #Add L2 loss
        loss += 0.5*reg_loss
        # Backward pass  
        ...
        #   # ...
        #   if i != self.num_layers:
        #       dhidden = dscores.matmul(self.params['W' + str(i + 1)].t())
        #       dhidden = dhidden * (torch.sign(F.relu(self.params['W' + str(i)])) + 1) / 2  # ReLU gradient
        #       if self.use_dropout:
        #           dhidden = dhidden * mask / (1 - self.dropout_param['p'])
        #       dscores = dhidden
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
              dx, grads['W' + str(i)], grads['b' + str(i)] = Linear.backward(dscores, cache[i])
            else:
              if self.use_dropout:
                # dx = dx * cache['dropout' + str(i)]
                dx = Dropout.backward(dx,cache['dropout'+str(i)] )
              dx, grads['W' + str(i)], grads['b' + str(i)] = Linear_ReLU.backward(dx, cache[i])
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=200, dtype=dtype, device=device,weight_scale=7e-2)
    #############################################################
...
def get_three_layer_network_params():
    ...
    ## not so good ##
    ## i have tried a list between learning_rates=[5e-1,5e-2,1e-2,5e-3,1e-3,5e-4,1e-4,5e-5] and weight_scales=[1e-1,5e-2,1e-2,5e-3,1e-3,5e-4,1e-4,5e-5]
    weight_scale = 9e-2  
...
def get_five_layer_network_params():
    ...
    ## not so good ##
    ## i have tried parameters same as previous function
    weight_scale = 9e-2  
    learning_rate = 1.2e-1 
    ################################################################
    #                       END OF YOUR CODE                       #
...
def adam(w, dw, config=None):
    ...
    config['v'] = config['beta2'] * config['v'] + (1 - config['beta2']) * dw **2 ## it depends on the norms, we used L2 norm
